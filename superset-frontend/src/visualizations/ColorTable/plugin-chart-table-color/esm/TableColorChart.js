...
function SortIcon({ column }) {
  ...
  if (isSorted) {
    sortIcon = isSortedDesc
      ? ___EmotionJSX(FaSortDesc, null)
      : ___EmotionJSX(FaSortAsc, null);
  }
...
export default function TableColorChart(props) {
  const {
    height,
    width,
    data,
  ...
  let colors = [];
  if (colorColumn) {
    data.map(d => colors.push(d[colorColumn]));
  }
      ...
      return {
        ...
        Cell: ({ value, row }) => {
          ...
          const cellProps = {
            ...
            style: {
              ...sharedStyle,
              color: colors[row.id] ? '#000000' : undefined,
  ...
  let cols = columnsMeta;
  if (colorColumn) {
    cols = cols.filter(d => d.key != colorColumn);
  }

  let columns = useMemo(() => cols.map(getColumnConfigs), [
    cols,
    getColumnConfigs,
  ]);
